...
| Artifact | Path | Use |
|---|---|---|
| **Single-file HTML app** | `build/dist/jiuwen-knowledge-offline.html` | Best phone/plane option: one file, no server, no internet. Search, tap-to-reveal, dark/light. |
...
| **EPUB** | `build/dist/jiuwen-knowledge.epub` | E-readers. |
| **Obsidian vault** | `build/dist/jiuwen-knowledge-vault.zip` | Open as a vault: one note per topic. |
...
**Windows**
- Read: open `build/dist/jiuwen-knowledge-offline.html`, or the site at `build/site/index.html`.
- Spaced repetition: install Anki, `File -> Import` the `.apkg` (or the CSV).

**Android**
- Copy `build/dist/jiuwen-knowledge-offline.html` to the phone and open it
...
1. **Local HTTP server (full search).** Build once, then:
   ...
   - Fully offline on the phone: install **Termux**, copy `build/dist/jiuwen-knowledge-site.zip`
...
3. **Obsidian (no build).** Open `build/dist/jiuwen-knowledge-vault.zip` as a vault:
