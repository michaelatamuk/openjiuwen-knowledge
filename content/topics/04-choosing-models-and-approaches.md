...
**Jiuwen:** `agent_evolving/evaluator/` has `FaithfulnessEvaluator`, `CorrectnessEvaluator`, and `LLMAsJudgeMetric` — a principled eval framework for quality measurement. `dev_tools/tune/Trainer` supports `early_stop_score` for automated quality gating. Gaps: cost and latency are not recorded *alongside* quality in the eval pipeline — there is no built-in cost-accuracy curve generation. Session costs are tracked in `usage_cost.py` but not correlated to per-query eval scores.
