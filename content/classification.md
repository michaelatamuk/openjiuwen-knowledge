...
|---|---|---|
| **basic** | Recall / definition / recognition. One concept, no tradeoff. | "What is X?", "What's the difference between A and B?", glossary terms, single facts. |
...
- Difficulty is about **how much reasoning**, not **how deep into code**. A
  `mechanism` question can be basic/intermediate.
- Do not derive it from `type` alone.

## Field on each entry (target shape)

```json
"meta": { "difficulty": "basic | intermediate | advanced", "tags": ["<topic-id>"] }
```
